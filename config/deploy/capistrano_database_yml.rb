...
Capistrano::Configuration.instance.load do
  ...
  namespace :deploy do
    ...
    namespace :db do
      ...
      task :delete_repo_database_yml, :except => { :no_release => true } do
        database_yml_file_to_delete = "#{release_path}/config/database.yml"
        run "if [ -f #{database_yml_file_to_delete} ]; then rm #{database_yml_file_to_delete}; fi"
      end
      ...
      task :setup, :except => { :no_release => true } do
        ...
        if template.nil?
          abort "missing database.yml template at #{location}"
        end
      ...
      task :symlink, :except => { :no_release => true } do
        run <<-COMMAND
          if [ ! -f #{release_path}/config/database.yml ]; then
